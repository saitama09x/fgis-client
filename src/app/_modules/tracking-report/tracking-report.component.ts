...
export class TrackingReportComponent implements OnInit {
  ...
  getClientTrackingFaceList() {
    if (this.startDate != null && this.endDate != null) {
      this.trackingService.getClientTrackingFaceList(this.clientID, this.startDate+' 00:00:00', this.endDate+' 23:59:59', this.pageSize, this.pageIndex, this.deviceUuid).subscribe(
        (data) => {
          this.faceDatas = data['faceDatas'];
          this.faceDataTotalCount = data['faceDataTotalCount'];

          this.faceDatas.forEach(data => {
            var stayDiff = Number(data.leave) - Number(data.enter);
            data.stayDuration = moment.utc(stayDiff).format("ss.SSS")+' seconds';
          });
        },
        err => {
          console.error(err);
